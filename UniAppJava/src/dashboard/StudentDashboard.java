...
import enrollment.MyEnrollmentsFrame;
import finance.PaymentHistoryFrame;
import java.awt.*;
import java.io.File;
import javax.swing.*;
import models.User;
...
public class StudentDashboard extends JFrame {
    ...
    public StudentDashboard(User user) {
        this.user = user;
        this.studentId = extractStudentId(user.getUsername());
        ...
        JLabel profileLabel = new JLabel("Hello, Student(" + user.getUsername() + ")!");
        ...
        // Four required cards
        cardPanel.add(createCard("My Enrollments", "Check your enrollment status", "enrollments.png",
                e -> new MyEnrollmentsFrame(studentId).setVisible(true)));

        cardPanel.add(createCard("My Payments", "Manage your fees and payments", "payments.png",
                e -> new PaymentHistoryFrame(studentId)));

        cardPanel.add(createCard("Exam Results", "View your exam results", "grades.png",
                e -> showMyResults()));

        cardPanel.add(createCard("My Profile", "View your personal information", "profile.png",
                e -> showProfile()));
    ...
    private int extractStudentId(String username) {
        return Integer.parseInt(username.strip());
    }

    private void showMyResults() {
        students.ResultsViewFrame resultsFrame = new students.ResultsViewFrame(studentId);
        resultsFrame.setVisible(true);
    }

    private void showProfile() {
        students.StudentDAO dao = new students.StudentDAO();
        models.Student student = dao.getStudentById(studentId);
        students.StudentDetailForm profileForm = new students.StudentDetailForm(this, student, "PROFILE");
